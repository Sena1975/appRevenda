        ...
        <form action="{{ route('tabelapreco.update', $tabela->id) }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="codfab" value="{{ $tabela->codfab }}" class="w-full border-gray-300 rounded-md shadow-sm">
                </div>

                {{-- Preço de Compra--}}
                <div>
                    <label class="block text-sm font-medium text-gray-700">Preço de Compra</label>
                    <input type="number" step="0.01" name="preco_compra" value="{{ $tabela->preco_compra }}"
                           class="w-full border-gray-300 rounded-md shadow-sm">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Preço de Revenda</label>
                    <input type="number" step="0.01" name="preco_revenda" value="{{ $tabela->preco_revenda }}"
