            ...
            <div class="min-w-max flex flex-nowrap items-end gap-3 pr-2">
                ...
                <div class="flex flex-col shrink-0">
                    ...
                    <select name="produto_id" id="produto_id"
                            class="h-10 w-80 border-gray-300 rounded-md shadow-sm">
                        <option value="">Todos</option>
                        @foreach($produtos as $p)
                            <option value="{{ $p->id }}" {{ (string)request('produto_id')===(string)$p->id?'selected':'' }}>
                                {{ $p->nome }} @if($p->codfabnumero) - {{ $p->codfabnumero }} @endif
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex flex-col shrink-0">
                    ...
                    <select name="status" id="status"
                            class="h-10 w-40 border-gray-300 rounded-md shadow-sm">
                        <option value="">Todos</option>
                ...
                <div class="flex flex-col shrink-0">
                    ...
                    <select name="vigencia" id="vigencia"
                            class="h-10 w-44 border-gray-300 rounded-md shadow-sm">
                ...
                <div class="flex flex-col shrink-0">
                    ...
                    <input type="text" id="busca" name="busca"
                           value="{{ request('busca') }}"
                           placeholder="Nome, codfab, número..."
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-3 py-2">
                            <div class="leading-tight">
                                ...
                                <div class="text-xs text-gray-500">
                                    @if(optional($item->produto)->codfab) codfab: {{ $item->produto->codfab }} @endif
                                    @if(optional($item->produto)->codfabnumero) • num: {{ $item->produto->codfabnumero }} @endif
                                </div>
                            </div>
                        </td>
                        <td class="px-3 py-2">R$ {{ number_format($item->preco_compra ?? 0, 2, ',', '.') }}</td>
                        <td class="px-3 py-2 font-semibold text-gray-900">R$ {{ number_format($item->preco_revenda, 2, ',', '.') }}</td>
                        ...
                        <td class="px-3 py-2 text-right">
                            <div class="inline-flex gap-2">
                                <a href="{{ route('tabelapreco.edit', $item->id) }}" class="rounded border px-2 py-1 hover:bg-gray-50">Editar</a>
                                <form action="{{ route('tabelapreco.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Excluir este registro?')">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="9" class="px-3 py-6 text-center text-gray-500">Nenhum registro encontrado.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
