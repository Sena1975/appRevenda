      ...
      <tbody>
        @forelse($contas as $c)
          @php
            ...
            $badge  = $status === 'PAGO' ? 'bg-green-100 text-green-700'
                   : ($status === 'CANCELADO' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700');
          @endphp
          <tr class="border-t hover:bg-gray-50">
            <td class="px-3 py-2">{{ $c->id }}</td>
            <td class="px-3 py-2">{{ $c->cliente_nome ?? '-' }}</td>
            ...
            <td class="px-3 py-2">
              <div class="flex items-center justify-center gap-2">
                <a href="{{ route('contasreceber.show', $c->id) }}" class="px-2 py-1 text-xs border rounded hover:bg-gray-50">Detalhes</a>

                @if($status !== 'PAGO' && $status !== 'CANCELADO')
                  <a href="{{ route('contasreceber.baixa', $c->id) }}" class="px-2 py-1 text-xs rounded bg-blue-600 text-white hover:bg-blue-700">
                    Baixar
                  </a>
                @endif

                <a href="{{ route('contasreceber.recibo', $c->id) }}" class="px-2 py-1 text-xs border rounded hover:bg-gray-50">
                  Recibo
                </a>
                ...
                @if($status === 'PAGO')
                  <form action="{{ route('contasreceber.estornar', $c->id) }}" method="POST"
                    ...
                    @csrf
                    <button type="submit" class="px-2 py-1 text-xs rounded bg-orange-600 text-white hover:bg-orange-700">
                      Estornar
                    </button>
                  </form>
                @endif
        ...
        @empty
          <tr><td colspan="7" class="px-3 py-6 text-center text-gray-500">Nenhum registro.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
