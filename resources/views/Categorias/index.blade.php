...
<div class="bg-white shadow rounded-lg p-6 max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-4">
        <p class="text-sm text-gray-500">Organize seus produtos por categorias.</p>
        <a href="{{ route('categorias.create') }}"
           class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">+ Nova Categoria</a>
    </div>
            ...
            <div class="min-w-max flex flex-nowrap items-end gap-3 pr-2">
                <div class="flex flex-col shrink-0">
                    ...
                    <input id="busca" name="busca" value="{{ request('busca') }}"
                           class="h-10 w-64 border-gray-300 rounded-md shadow-sm" placeholder="Nome da categoria">
                </div>
                <div class="flex flex-col shrink-0">
                ...
                <div class="flex flex-col shrink-0">
                    ...
                    <select id="por_pagina" name="por_pagina"
                            class="h-10 w-36 border-gray-300 rounded-md shadow-sm" onchange="this.form.submit()">
                ...
                <div class="flex items-end gap-2 shrink-0">
                    <button class="h-10 rounded-md bg-blue-600 px-4 text-white hover:bg-blue-700 transition" type="submit">Filtrar</button>
                    <a href="{{ route('categorias.index') }}"
                       class="h-10 rounded-md border px-4 text-gray-700 hover:bg-gray-50 transition">Limpar</a>
                </div>
            </div>
        </div>
    </form>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-3 py-2 text-left font-medium text-gray-600">Categoria (texto livre)</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($categorias as $c)
                    <tr class="hover:bg-gray-50">
                        <td class="px-3 py-2 font-medium text-gray-900">{{ $c->nome }}</td>
                        <td class="px-3 py-2">{{ $c->categoria ?? '—' }}</td>
                        <td class="px-3 py-2">
                            <span class="px-2 py-1 rounded text-white {{ $c->status ? 'bg-green-500' : 'bg-red-500' }}">
                ...
                @empty
                    <tr><td colspan="4" class="px-3 py-6 text-center text-gray-500">Nenhuma categoria encontrada.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
