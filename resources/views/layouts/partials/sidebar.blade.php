            ...
            <div x-show="open" x-collapse class="mt-1 space-y-1 pl-8">
                ...
                <div class="flex items-center">
                    @if ($qtdPendentesVenda > 0)
                        <a x-show="openSidebar"
                           href="{{ route('vendas.index', ['status' => 'PENDENTES']) }}"
                           ...
                           title="Ver pedidos pendentes (PENDENTE + ABERTO)">
                            Pendentes
                            <span class="ml-1 align-middle text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">
                                {{ $qtdPendentesVenda }}
                            </span>
                        </a>
                    @endif
            ...
            <div x-show="open" x-collapse class="mt-1 space-y-1 pl-8">
                @php
                    $qtdFormas = class_exists(\App\Models\FormaPagamento::class)
                        ? \App\Models\FormaPagamento::when($empresaId, fn ($q) => $q->where('empresa_id', $empresaId))->count()
                        : 0;

                    $qtdPlanos = class_exists(\App\Models\PlanoPagamento::class)
                        ? \App\Models\PlanoPagamento::when($empresaId, fn ($q) => $q->where('empresa_id', $empresaId))->count()
                        : 0;
