@php
    ...
    // Contadores dinâmicos (por empresa)
    ...
    $isPedidos = request()->routeIs('compras.*', 'vendas.*', 'relatorios.*');
    ...
    $isCampanhas = request()->routeIs('campanhas.*', 'campanhas.restricoes*', 'participacoes.*', 'indicacoes.*');

    // Novo grupo: Mensageria (Modelos + Config WhatsApp)
    $isMensageria = request()->routeIs(
        'mensageria.*', // 👈 aqui era 'mensagens.*'
        'whatsapp-config.*',
    );
@endphp
...
<div x-data="{ openSidebar: true }" class="flex flex-col h-full border-r bg-white shadow-sm transition-all duration-300"
    ...
    <nav class="flex-1 overflow-y-auto px-2 py-4 text-sm font-medium text-gray-700">
        ...
        <a href="{{ route('dashboard') }}"
            class="flex items-center px-3 py-2 rounded hover:bg-blue-50 transition-all duration-200
                  {{ request()->routeIs('dashboard') ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-700' }}">
        ...
        <div x-data="{ open: {{ $isCadastro ? 'true' : 'false' }} }" class="mt-2">
            ...
            <div x-show="open" x-collapse class="mt-1 space-y-1 pl-8">
                ...
                @foreach ($cadastros as [$titulo, $rota, $svg])
                    @if (Route::has($rota))
                        <a href="{{ route($rota) }}"
                            class="flex items-center px-2 py-1 rounded hover:bg-blue-100 transition-all duration-150
                                  {{ request()->routeIs(Str::before($rota, '.') . '.*') ? 'text-blue-700 font-semibold' : 'text-gray-600' }}">
        ...
        <div x-data="{ open: {{ $isPedidos ? 'true' : 'false' }} }" class="mt-2">
            ...
            <div x-show="open" x-collapse class="mt-1 space-y-1 pl-8">
                ...
                {{-- Relatório: Entrada de Mercadoria --}}
                <div class="flex items-center justify-between px-2 py-1 rounded hover:bg-blue-100 transition-all">
                    <a href="{{ route('relatorios.entrada-mercadoria') }}"
                        class="flex items-center {{ request()->routeIs('relatorios.entrada-mercadoria') ? 'text-blue-700 font-semibold' : 'text-gray-600' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 7h18M7 3v4m10-4v4M5 11h14v10H5V11z" />
                        </svg>
                        <span x-show="openSidebar" class="ml-2">Entrada de Mercadoria</span>
                    </a>
                </div>
                ...
                {{-- Relatório: Rentabilidade de Vendas --}}
                <div class="flex items-center justify-between px-2 py-1 rounded hover:bg-blue-100 transition-all">
                    <a href="{{ route('relatorios.rentabilidade-vendas') }}"
                        class="flex items-center {{ request()->routeIs('relatorios.rentabilidade-vendas') ? 'text-blue-700 font-semibold' : 'text-gray-600' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 19V5M4 19h16M8 15l3-3 3 2 4-5" />
                        </svg>
                        <span x-show="openSidebar" class="ml-2">Rentabilidade</span>
                    </a>
                </div>
                {{-- Relatório: Vendas Detalhadas --}}
                <div class="flex items-center justify-between px-2 py-1 rounded hover:bg-blue-100 transition-all">
                    <a href="{{ route('relatorios.vendas-detalhadas') }}"
                        class="flex items-center {{ request()->routeIs('relatorios.vendas-detalhadas') ? 'text-blue-700 font-semibold' : 'text-gray-600' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 17v-2m3 2v-6m3 6V7M4 19h16" />
                        </svg>
                        <span x-show="openSidebar" class="ml-2">Vendas Detalhadas</span>
                    </a>
                </div>
                ...
                <div class="flex items-center">
                    @if ($qtdPendentesVenda > 0)
                        <a x-show="openSidebar" href="{{ route('vendas.index', ['status' => 'PENDENTES']) }}"
                            class="ml-2 shrink-0 whitespace-nowrap text-[11px] px-2 py-0.5 rounded-full
            ...
            <div x-show="open" x-collapse class="mt-1 space-y-1 pl-8">
                @php
                    $qtdFormas = class_exists(\App\Models\FormaPagamento::class)
                        ? \App\Models\FormaPagamento::when(
                            $empresaId,
                            fn($q) => $q->where('empresa_id', $empresaId),
                        )->count()
                        : 0;
                    ...
                    $qtdPlanos = class_exists(\App\Models\PlanoPagamento::class)
                        ? \App\Models\PlanoPagamento::when(
                            $empresaId,
                            fn($q) => $q->where('empresa_id', $empresaId),
                        )->count()
                        : 0;
                ...
                <a href="{{ route('formapagamento.index') }}"
                    class="flex items-center justify-between px-2 py-1 rounded hover:bg-blue-100 transition-all
                          {{ request()->routeIs('formapagamento.*') ? 'text-blue-700 font-semibold' : 'text-gray-600' }}">
                    <div class="flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if ($qtdFormas > 0)
                        <span x-show="openSidebar" class="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full">
                            {{ $qtdFormas }}
                        </span>
                    @endif
                </a>
                ...
                <a href="{{ route('planopagamento.index') }}"
                    class="flex items-center justify-between px-2 py-1 rounded hover:bg-blue-100 transition-all
          {{ request()->routeIs('planopagamento.*') ? 'text-blue-700 font-semibold' : 'text-gray-600' }}">
                    <div class="flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('contasreceber.index') }}"
                    class="flex items-center justify-between px-2 py-1 rounded hover:bg-blue-100 transition-all
                          {{ request()->routeIs('contasreceber.*') ? 'text-blue-700 font-semibold' : 'text-gray-600' }}">
                    <div class="flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('contaspagar.index') }}"
                    class="flex items-center justify-between px-2 py-1 rounded hover:bg-blue-100 transition-all
                          {{ request()->routeIs('contaspagar.*') ? 'text-blue-700 font-semibold' : 'text-gray-600' }}">
                    <div class="flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if ($qtdPagar > 0)
                        <span x-show="openSidebar" class="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">
                            {{ $qtdPagar }}
                        </span>
                    @endif
                </a>
