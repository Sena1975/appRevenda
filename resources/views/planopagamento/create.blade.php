{{-- resources/views/planopagamento/create.blade.php --}}
@extends('layouts.app')

@section('content')
<div class="max-w-3xl mx-auto p-6">
    <h1 class="text-2xl font-bold mb-4">Novo Plano de Pagamento</h1>
    ...
    <form action="{{ route('planopagamento.store') }}" method="POST" class="bg-white border rounded p-4">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium mb-1">Código do Plano *</label>
                <input type="text" name="codplano" value="{{ old('codplano') }}"
            ...
            <div>
                ...
                <select name="formapagamento_id" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($formas as $f)
                        <option value="{{ $f->id }}" @selected(old('formapagamento_id')==$f->id)>{{ $f->nome }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" min="1" name="parcelas" value="{{ old('parcelas') }}"
                       class="w-full border rounded px-3 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Prazo 1 (dias)</label>
                <input type="number" min="0" name="prazo1" value="{{ old('prazo1') }}"
                       class="w-full border rounded px-3 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Prazo 2 (dias)</label>
                <input type="number" min="0" name="prazo2" value="{{ old('prazo2') }}"
                       class="w-full border rounded px-3 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Prazo 3 (dias)</label>
                <input type="number" min="0" name="prazo3" value="{{ old('prazo3') }}"
                       class="w-full border rounded px-3 py-2">
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Prazo Médio (dias)</label>
                <input type="number" min="0" name="prazomedio" value="{{ old('prazomedio') }}"
                       class="w-full border rounded px-3 py-2">
            </div>

            <div class="flex items-center gap-2 mt-2">
                <input type="checkbox" name="ativo" value="1" id="ativo" class="h-4 w-4" {{ old('ativo',1) ? 'checked' : '' }}>
