...
<x-app-layout>
        ...
        <form action="{{ route('clientes.update', $cliente->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="nome" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('nome', $cliente->nome) }}" required>
                </div>

                <!-- CPF -->
                <div>
                    <label class="block text-sm font-medium text-gray-700">CPF</label>
                    <input type="text" name="cpf" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('cpf', $cliente->cpf) }}" placeholder="000.000.000-00">
                </div>

                <!-- Telefone -->
                <div>
                    <label class="block text-sm font-medium text-gray-700">Telefone</label>
                    <input type="text" name="telefone" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('telefone', $cliente->telefone) }}" placeholder="(xx) xxxx-xxxx">
                </div>

                <!-- WhatsApp -->
                <div>
                    <label class="block text-sm font-medium text-gray-700">WhatsApp</label>
                    <input type="text" name="whatsapp" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('whatsapp', $cliente->whatsapp) }}" placeholder="(xx) 9xxxx-xxxx">
                </div>

                <!-- Telegram -->
                <div>
                    <label class="block text-sm font-medium text-gray-700">Telegram</label>
                    <input type="text" name="telegram" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('telegram', $cliente->telegram) }}" placeholder="@usuario">
                </div>

                <!-- Instagram -->
                <div>
                    <label class="block text-sm font-medium text-gray-700">Instagram</label>
                    <input type="text" name="instagram" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('instagram', $cliente->instagram) }}" placeholder="@usuario">
                </div>

                <!-- Facebook -->
                <div>
                    <label class="block text-sm font-medium text-gray-700">Facebook</label>
                    <input type="text" name="facebook" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('facebook', $cliente->facebook) }}" placeholder="facebook.com/usuario">
                </div>

                <!-- Email -->
                <div>
                    <label class="block text-sm font-medium text-gray-700">E-mail</label>
                    <input type="email" name="email" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('email', $cliente->email) }}">
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="cep" id="cep" class="w-full border-gray-300 rounded-md shadow-sm pr-10" value="{{ old('cep', $cliente->cep) }}" placeholder="00000-000">
                ...
                <div>
                    ...
                    <input type="text" name="endereco" id="endereco" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('endereco', $cliente->endereco) }}" placeholder="Rua, avenida, número...">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="data_nascimento" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('data_nascimento', $cliente->data_nascimento) }}">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="filhos" min="0" class="w-full border-gray-300 rounded-md shadow-sm" value="{{ old('filhos', $cliente->filhos ?? 0) }}">
                </div>

                <!-- Time do Coração (select + "Outro") -->
                @php
                    $timesLista = [
                        'Sudeste' => ['Flamengo','Vasco','Botafogo','Fluminense','Corinthians','Palmeiras','Santos','São Paulo','Cruzeiro','Atlético Mineiro'],
                    ...
                    foreach ($timesLista as $grupo => $arr) {
                        if (in_array($valorTime, $arr)) { $estaNaLista = true; break; }
                    }
                @endphp
                    ...
                    <select id="time_select" class="w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        @foreach($timesLista as $grupo => $arr)
                            <optgroup label="{{ $grupo }}">
                                @foreach($arr as $t)
                                    <option value="{{ $t }}" @selected($estaNaLista && $valorTime === $t)>{{ $t }}</option>
                                @endforeach
                            </optgroup>
                        @endforeach
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <input type="file" name="foto" id="foto" accept="image/*" class="block text-sm text-gray-700 cursor-pointer">
    ...
    document.addEventListener('DOMContentLoaded', async function () {
        ...
        async function carregarCidades(ufId, selecionarPorNome = null) {
            ...
            cidades.forEach(c => {
                const match = selecionarPorNome && c.nome.trim().toLowerCase() === selecionarPorNome.trim().toLowerCase();
                cidadeSelect.insertAdjacentHTML('beforeend', `<option value="${c.id}" ${match ? 'selected' : ''}>${c.nome}</option>`);
        ...
        async function carregarBairros(cidadeId, selecionarPorNome = null) {
            ...
            let marcou = false;
            bairros.forEach(b => {
                const match = selecionarPorNome && b.nome.trim().toLowerCase() === selecionarPorNome.trim().toLowerCase();
                bairroSelect.insertAdjacentHTML('beforeend', `<option value="${b.id}" ${match ? 'selected' : ''}>${b.nome}</option>`);
                if (match) marcou = true;
            });
            if (!marcou && selecionarPorNome) {
                // cria "custom"
                bairroSelect.insertAdjacentHTML('beforeend', `<option value="custom" selected>${selecionarPorNome}</option>`);
        ...
        if (ufAtualSigla) {
            const ufOption = [...ufSelect.options].find(o => (o.dataset.sigla || '').toUpperCase() === ufAtualSigla.toUpperCase());
            if (ufOption) {
        ...
        cepInput?.addEventListener('blur', async function () {
            ...
            try {
                ...
                const ufOption = [...ufSelect.options].find(o => (o.dataset.sigla || '').toUpperCase() === ufSigla.toUpperCase());
                if (!ufOption) {
