{{-- resources/views/subcategorias/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-700">Editar Subcategoria</h2>
    </x-slot>
        ...
        <form action="{{ route('subcategorias.update', $subcategoria) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="categoria_id" class="w-full border-gray-300 rounded-md shadow-sm" required>
                        @foreach($categorias as $cat)
                            <option value="{{ $cat->id }}"
                                {{ (string)old('categoria_id', $subcategoria->categoria_id)===(string)$cat->id ? 'selected' : '' }}>
                                {{ $cat->nome }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="subcategoria" value="{{ old('subcategoria', $subcategoria->subcategoria) }}"
                ...
                <div>
                    ...
                    <select name="status" class="w-full border-gray-300 rounded-md shadow-sm">
                        <option value="1" {{ (int)old('status', $subcategoria->status)===1 ? 'selected' : '' }}>Ativo</option>
                        <option value="0" {{ (int)old('status', $subcategoria->status)===0 ? 'selected' : '' }}>Inativo</option>
                    </select>
                </div>
            </div>
