    ...
    <tr class="border-b hover:bg-gray-50">
        <td class="px-3 py-2">
            <div class="flex items-center gap-2">
                <span class="font-medium text-gray-800">{{ $rev->nome }}</span>

                @if($rev->revenda_padrao) {{-- boolean/cast no Model ajuda aqui --}}
                    <span class="inline-flex items-center px-2 py-0.5 rounded bg-green-100 text-green-800 text-xs font-medium">
                        Padrão
                    </span>
                @endif
            </div>
        </td>                        <td class="px-4 py-2">{{ $rev->cpf }}</td>
                        ...
                        <td class="px-4 py-2 text-right space-x-2">
                            ...
                            <form action="{{ route('revendedoras.destroy', $rev->id) }}" method="POST"
                                class="inline-block"
