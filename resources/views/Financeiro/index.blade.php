...
<div class="max-w-7xl mx-auto p-6">
  ...
  <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-3 bg-white p-4 rounded border mb-4">
    <div>
      ...
      <input type="text" name="cliente" class="w-full border rounded"
             value="{{ $filtro['cliente'] ?? '' }}">
    </div>

    <div>
      <label class="block text-sm">Status</label>
      @php $st = $filtro['status'] ?? 'TODOS'; @endphp
    ...
    <div>
      ...
      <input type="date" name="dt_ini" class="w-full border rounded"
             value="{{ $filtro['dt_ini'] ?? '' }}">
    </div>

    <div>
      <label class="block text-sm">Até</label>
      <input type="date" name="dt_fim" class="w-full border rounded"
             value="{{ $filtro['dt_fim'] ?? '' }}">
    </div>
          ...
          <tr class="border-t hover:bg-gray-50">
            ...
            <td class="px-3 py-2">
              <div class="flex items-center justify-center gap-2">
                ...
                @if($status === 'PAGO')
                  <form action="{{ route('contasreceber.estornar', $c->id) }}"
                        method="POST"
