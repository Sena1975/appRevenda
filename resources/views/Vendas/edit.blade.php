        ...
        <form action="{{ route('vendas.update', $pedido->id) }}" method="POST" id="formVenda">
                    ...
                    <select name="plano_pagamento_id" id="planoPagamento" class="w-full border rounded" required>
                        ...
                        @forelse($planos as $p)
                            <option value="{{ $p->id }}" data-codigo="{{ $p->codigo ?? '' }}"
                                @selected($pedido->plano_pagamento_id == $p->id)>
            ...
            <div class="overflow-x-auto mb-4">
                <table class="min-w-full border table-fixed" id="tblItens">
                    <colgroup>
                        <col style="width: 50%"> {{-- Produto --}}
                        <col style="width: 10%"> {{-- Qtd   --}}
                        <col style="width: 10%"> {{-- Pontos --}}
                        <col style="width: 15%"> {{-- R$ Unit --}}
                        <col style="width: 15%"> {{-- R$ Total --}}
                            ...
                            <tr class="linha border-t">
                                <td class="px-2 py-2 max-w-md align-top">
                                    ...
                                    <input type="hidden" name="itens[{{ $idx }}][codfabnumero]"
                                        class="codfab-hidden" value="{{ $it->codfabnumero }}">
                                    <input type="hidden" name="itens[{{ $idx }}][pontuacao]"
                                ...
                                <td class="px-2 py-2">
                                    <input type="number" min="1" step="1" value="{{ (int) $it->quantidade }}"
                                        ...
                                        class="quantidade w-full border rounded text-right" inputmode="numeric"
                                        pattern="\d*">
                                </td>
                                <td class="px-2 py-2">
                                    <input type="number" min="0" step="1"
                                        class="pontos-unit w-full border rounded text-right"
                                        value="{{ (int) $it->pontuacao }}">
                                </td>

                                <td class="px-2 py-2">
                                    <input type="number" min="0" step="0.01"
                                        value="{{ number_format($it->preco_unitario, 2, '.', '') }}"
                                        name="itens[{{ $idx }}][preco_unitario]"
                                        class="preco-unit w-full border rounded text-right">
                                </td>
                                <td class="px-2 py-2">
                                    <input type="number" min="0" step="0.01"
                                        value="{{ number_format($it->preco_total, 2, '.', '') }}"
                    ...
                    <tfoot>
                        <tr class="bg-gray-50">
                            ...
                            <td class="px-2 py-2">
                                <input type="number" step="0.01" name="valor_total" id="totalBruto"
                        ...
                        <tr>
                            ...
                            <td class="px-2 py-2">
                                <input type="number" step="0.01" name="valor_desconto" id="totalDesc"
                        ...
                        <tr class="bg-gray-50">
                            ...
                            <td class="px-2 py-2">
                                <input type="number" step="0.01" name="valor_liquido" id="totalLiq"
                                    class="w-full border rounded text-right" readonly
                    ...
                    <tr class="linha border-t">
                        <td class="px-2 py-2 max-w-md align-top">
                            ...
                            <input type="hidden" name="itens[__IDX__][codfabnumero]" class="codfab-hidden"
                                value="">
                            <input type="hidden" name="itens[__IDX__][pontuacao]" class="pontos-unit-hidden"
                                value="0">
                        ...
                        <td class="px-2 py-2">
                            <input type="number" min="1" step="1" value="1"
                                name="itens[__IDX__][quantidade]" class="quantidade w-full border rounded text-right"
                        ...
                        <td class="px-2 py-2">
                            <input type="number" min="0" step="0.01" value="0.00"
                                name="itens[__IDX__][preco_unitario]" class="preco-unit w-full border rounded text-right">
                        </td>

                        <td class="px-2 py-2">
                            <input type="number" min="0" step="0.01" value="0.00"
                                name="itens[__IDX__][preco_total]" class="preco-total w-full border rounded text-right"
                                readonly>
                        </td>
            ...
            <input type="hidden" name="pontuacao_total" id="pedidoPontuacaoTotal"
                value="{{ (int) $pedido->pontuacao_total }}">
                ...
                <div class="bg-white/95 backdrop-blur border-t px-4 py-3 rounded-t">
                    <div class="max-w-6xl mx-auto flex items-center justify-end gap-2">
                        ...
                        <span class="mr-auto text-sm">
                            ...
                            @if ($st === 'ENTREGUE')
                                <span
                                    class="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-green-700">Entregue</span>
                            @elseif($st === 'PENDENTE' || $st === 'ABERTO')
                                <span
                                    class="inline-flex items-center rounded-full bg-yellow-100 px-2 py-0.5 text-yellow-700">Pendente</span>
                            @else
                                <span
                                    class="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-gray-700">{{ $pedido->status }}</span>
                            @endif
                        </span>

                        {{-- Botão Confirmar entrega (só se não estiver ENTREGUE) --}}
                        @if (!in_array(strtoupper($pedido->status), ['ENTREGUE']))
                            <button type="button" onclick="confirmarEntregaPedido()"
                                class="px-3 py-2 rounded-md bg-blue-600 text-white font-semibold
    ...
    <script>
        (function() {
            ...
            async function carregarPlanos(formaId, selectedPlanoId = null) {
                ...
                try {
                    ...
                    (data || []).forEach(p => {
                        ...
                        const sel = (selectedPlanoId && Number(selectedPlanoId) === Number(p.id)) ?
                            'selected' : '';
            ...
            document.addEventListener('change', function(e) {
                ...
                if (
                    el.classList.contains('quantidade') ||
                    el.classList.contains('preco-unit') ||
                    el.classList.contains('pontos-unit')
                ) {
            ...
            document.addEventListener('input', function(e) {
                ...
                if (el.classList.contains('pontos-unit')) {
                    const tr = el.closest('tr.linha');
                    if (tr) {
                        recalcularLinha(tr);
                        recalcularTotais();
